...
│   ├── toolExecution.ts      # 工具执行 (1651 行)
│   ├── StreamingToolExecutor.ts # 流式执行器
│   └── toolHooks.ts          # 工具钩子
├── compact/                  # 紧凑服务层 (13 文件)
│   ├── compact.ts            # 对话压缩核心 (1579 行)
│   ├── autoCompact.ts        # 自动压缩
...
├── lsp/                      # LSP 服务层 (7 文件)
│   ├── LSPClient.ts          # LSP 客户端 (400 行)
│   ├── manager.ts            # LSP 管理器 (261 行)
│   └── ...
├── analytics/                # 分析服务层
├── oauth/                    # OAuth 服务层
...
### 2.2 工具执行核心 (toolExecution.ts - 1651 行)
...
### 2.3 流式工具执行器 (StreamingToolExecutor.ts - 481 行)
...
### 3.4 API 微压缩 (apiMicrocompact.ts - 134 行)
...
### 3.5 会话内存压缩 (sessionMemoryCompact.ts - 568 行)
...
### 4.1 LSP 客户端 (LSPClient.ts - 400 行)
