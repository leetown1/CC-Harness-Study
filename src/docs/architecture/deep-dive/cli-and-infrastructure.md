...
src/
├── cli/                          # CLI 核心实现 (19 文件)
│   ├── update.ts                 # 自动更新机制 (383 行)
│   ├── exit.ts                   # 退出辅助函数
│   ├── print.ts                  # 主输出处理 (5256 行)
│   ├── structuredIO.ts           # 结构化 I/O (811 行)
│   ├── remoteIO.ts               # 远程 I/O (234 行)
...
│   └── transports/               # 通信传输层
│       ├── WebSocketTransport.ts # WebSocket 传输 (707 行)
│       ├── SSETransport.ts       # SSE 传输 (620 行)
│       ├── HybridTransport.ts    # 混合传输
│       ├── ccrClient.ts          # CCR v2 客户端 (929 行)
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `config.ts` | 1607 | 配置管理、级联、监听 |
| `auth.ts` | 1782 | 认证、OAuth、API 密钥管理 |
